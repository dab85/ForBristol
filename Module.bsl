...
//da20250316 Экспериментальные функции{
Функция ПолучитьРасходныеОрдерыНаДень()Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	БФ_РасходникиНаСкладСрезПоследних.Документ КАК document,
	|	БФ_РасходникиНаСкладСрезПоследних.Документ.Номер КАК number,
	|	БФ_РасходникиНаСкладСрезПоследних.Документ.Дата КАК date
	|ИЗ
	|	РегистрСведений.БФ_РасходникиНаСклад.СрезПоследних(&Дата, ) КАК БФ_РасходникиНаСкладСрезПоследних";
	...
	Пока Выборка.Следующий()Цикл
		...
		//Структура.Вставить("document", Строка(Выборка.document));
		Структура.Вставить("number", Строка(Выборка.number));
		Структура.Вставить("date", Строка(Выборка.date));
...
Функция ПолучитьРасходныйОрдерПоНомеру(Номер)Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = 	
	"ВЫБРАТЬ
	|	РасходныйОрдерНаТоварыОтгружаемыеТовары.Номенклатура КАК product,
	|	РасходныйОрдерНаТоварыОтгружаемыеТовары.КоличествоУпаковок КАК countbox,
	|	РасходныйОрдерНаТоварыОтгружаемыеТовары.БФ_КоличествоУпаковокФакт КАК count,
	...
	|ГДЕ
	|	РасходныйОрдерНаТоварыОтгружаемыеТовары.Ссылка.Номер = &Номер"  ;
	...
	Пока Выборка.Следующий()Цикл  
		Структура = Новый Структура();
		//Структура.Вставить("document", Строка(Выборка.document));
		Структура.Вставить("number", Строка(Выборка.Ссылка.Номер));
		Структура.Вставить("date", Строка(Выборка.Ссылка.Дата));
		
		СтруктураТаблица = Новый Структура();
		//Структура.Вставить("document", Строка(Выборка.document));
		СтруктураТаблица.Вставить("product", Строка(Выборка.product));
		СтруктураТаблица.Вставить("count", Строка(Выборка.count));
		СтруктураТаблица.Вставить("countbox", Строка(Выборка.countbox));
		Массив.Добавить(СтруктураТаблица);
		Структура.Вставить("table", Массив);
	КонецЦикла;
...
Функция НайтиНоменклатуруПоШКТекстом(Штрихкод) Экспорт
	...
	|		ПО ШтрихкодыНоменклатуры.Упаковка.Ссылка = УпаковкиЕдиницыИзмерения.Ссылка
	|ГДЕ
	|	ШтрихкодыНоменклатуры.Штрихкод = &Штрихкод";
...
Функция НайтиКоличествоВУпаковкеПоШК(Штрихкод) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ШтрихкодыНоменклатуры.Упаковка.Числитель КАК КоличествоВУпаковке,
	|	ШтрихкодыНоменклатуры.Номенклатура.Артикул КАК Артикул,
	|	ШтрихкодыНоменклатуры1.Штрихкод КАК Штрихкод
	|ИЗ
	|	РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	...
	|ГДЕ
	|	ШтрихкодыНоменклатуры.Штрихкод = &Штрихкод
	|	И ШтрихкодыНоменклатуры1.Упаковка.Числитель = 1";
	...
	СтруктураПустая = Новый Структура;
	СтруктураПустая.Вставить("КоличествоВУпаковке", 0);	
	СтруктураПустая.Вставить("Артикул", "");	
	СтруктураПустая.Вставить("Штрихкод", "");	
...
Функция СоздатьотборРазмещение(Параметры)Экспорт
	...
	Если  Не ЯчейкаРазмещения.Пустая() И  Не Упаковка.Пустая() Тогда
		...
		ДокументОтбор.Дата = ТекущаяДата(); 
